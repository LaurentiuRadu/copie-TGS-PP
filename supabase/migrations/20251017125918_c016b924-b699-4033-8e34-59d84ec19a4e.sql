-- ========================================
-- TRIGGER: Sincronizare automată pontaje coordonatori/șefi de echipă
-- Când un coordonator/șef de echipă este editat într-o echipă,
-- modificările se propagă automat în toate echipele unde apare pentru aceeași zi
-- ========================================
...
-- Trigger care se activează la UPDATE pe time_entries
DROP TRIGGER IF EXISTS trigger_sync_management_entries ON time_entries;
CREATE TRIGGER trigger_sync_management_entries
AFTER UPDATE ON time_entries
FOR EACH ROW
WHEN (OLD.clock_in_time IS DISTINCT FROM NEW.clock_in_time 
      OR OLD.clock_out_time IS DISTINCT FROM NEW.clock_out_time
      OR OLD.approval_status IS DISTINCT FROM NEW.approval_status)
EXECUTE FUNCTION sync_management_time_entries();